...
import { navbarStyles } from '../assets/assets1/dummyStyles'
import { Link, useLocation, useNavigate } from 'react-router-dom';
import {  SignInButton, SignOutButton, useClerk, UserButton } from '@clerk/react';
...
//close the toggle menu when click outside
  useEffect(() => {
    ...
    <nav  ref={navRef} className={`${navbarStyles.navbarContainer} ${showNavbar ? navbarStyles.navbarVisible : navbarStyles.navbarHidden} `}>
      <div className={navbarStyles.contentWrapper}>
        <div className={navbarStyles.flexContainer}>
          {/* Logo */}
          <Link className={navbarStyles.logoLink} to='/'>
        ...
        {isOpen && (
          <div className={navbarStyles.mobileMenu}>
            {navItems.map((item, idx)=>{
              const isActive= location.pathname === item.href;
              return (
                <Link  to={item.href} key={idx} onClick={()=> setIsOpen(false)} className={`${navbarStyles.mobileMenuItem} ${isActive ? navbarStyles.mobileMenuItemActive : navbarStyles.mobileMenuItemInactive}`}>
                  {item.label}
